...
namespace _Project.Scripts.SceneManagement
{
    public class SceneGroupManager
    {
        ...
        public async Task LoadScenes(SceneGroup group, IProgress<float> progress, bool reloadDupScenes = false)
        {
            ...
            int totalScenesToLoad = _activeSceneGroup._scenes.Count;

            AsyncOperationGroup operationGroup = new AsyncOperationGroup(totalScenesToLoad);

            for (int i = 0; i < totalScenesToLoad; i++)
            {
                SceneData sceneData = group._scenes[i];
                if (!reloadDupScenes && loadedScenes.Contains(sceneData.Name)) continue;

                AsyncOperation operation = SceneManager.LoadSceneAsync(sceneData._reference.Path, LoadSceneMode.Additive);
